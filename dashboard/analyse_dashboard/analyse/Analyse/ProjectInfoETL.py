...
from Analyse.ETL import Load, Transform, logger
...
class ProjectInfoExtract(FttXExtract):
    ...
    def __init__(self, **kwargs):
        ...
        self.location_of_project_info_excel = kwargs["config"].get(
    ...
    def _get_map_bnumber_vs_project_from_fc(self):
        ds = (
            self.extracted_data.df[["projectcode", "project"]]
            .set_index("projectcode")
            .drop_duplicates()
        )
    ...
    def get_project_info_record(self, client):
        record = (
            ...
            .document(f"{client}_project_dates")
            .get()
            .to_dict()
        )
        return record
...
class ProjectInfoTransform(Transform):
    ...
    def transform(self, **kwargs):
        ...
        df_updated = self.update_project_info(
            df, df_newinfo, map_bnumber_vs_projectname
        )
    ...
    def _make_table_from_excel_project_info(self):
        ...
        if self.client_name == "kpn":
            df = pd.read_excel(xls, "KPN")
            name_col_projectname = [
                el for el in df.columns if "werkvoorbereidingsplanning" in el
            ][0]
            df = df[
                [
                    "KPN B nr.",
            ...
            raise NotImplementedError(
                "This function needs to be extended for tmobile and dfn"
            )
        ...
        df["snelheid (m/week)"] = (
            df["meters BIS"] / df["doorlooptijd"].astype(float)
        ).round()
    ...
    def _make_table_from_record_project_info(self):
        ...
        df["snelheid (m/week)"] = (
            df["meters BIS"] / (df["FTU1"] - df["FTU0"]).dt.days * 7
        ).round()
        return df
...
class ProjectInfoLoad(Load):
    ...
    def load(self, **kwargs):
        ...
        firestore.Client().collection("ProjectInfo").document(
            f"{self.client_name}_project_dates"
        ).set(record)
