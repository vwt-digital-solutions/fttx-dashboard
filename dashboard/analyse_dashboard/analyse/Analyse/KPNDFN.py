...
from Analyse.FttX import (FttXAnalyse, FttXETL, FttXExtract, FttXLocalETL,
                          FttXTestLoad, FttXTransform, PickleExtract)
...
from functions import (calculate_week_target, error_check_FCBC,
                       extract_bis_target_project, get_start_time,
                       get_timeline, get_total_objects,
                       lastweek_realisatie_hpend_bullet_chart,
                       make_graphics_for_number_errors_fcbc_per_project,
                       make_graphics_for_ratio_hc_hpend_per_project,
                       performance_matrix, prognose, prognose_graph,
                       targets_new, thisweek_realisatie_hpend_bullet_chart,
                       week_realisatie_bullet_chart)
...
class KPNAnalyse(FttXAnalyse):
    ...
    def _prognose(self):
        ...
        d_real_l_r = {
            k: v["Aantal"] for k, v in self.intermediate_results.d_real_l.items()
        }
        self.records.add("d_real_l_r", d_real_l_r, ListRecord, "Data")
        d_real_l_ri = {
            k: v.index for k, v in self.intermediate_results.d_real_l.items()
        }
    ...
    def _targets(self):
        ...
        y_target_l, t_diff, target_per_week_dict = targets_new(
    ...
    def _calculate_project_indicators(self):
        logger.info(
            "Calculating project indicators and making graphic boxes for dashboard"
        )
        ...
        for project in self.project_list:
            ...
            week_target = calculate_week_target(
                project=project,
            ...
            lastweek_target = calculate_week_target(
                project=project,
                target_per_week=self.intermediate_results.target_per_week,
                FTU0=self.transformed_data.ftu["date_FTU0"],
                FTU1=self.transformed_data.ftu["date_FTU1"],
            ...
            project_df = self.transformed_data.df[
                self.transformed_data.df.project == project
            ]

            # bis realisatie and target dates
            bis_realisatie_dates = project_df[
                br.bis_opgeleverd(project_df)
            ].status_civiel_datum
            bis_realisatie_dates = (
                bis_realisatie_dates.value_counts().resample("D").sum()
            )
            bis_target_dates = extract_bis_target_project(
                civiel_startdatum=self.transformed_data.get("civiel_startdatum").get(
                    project
                ),
                total_meters_bis=self.transformed_data.get("total_meters_bis").get(
                    project
                ),
            ...
            # bis realisaties
            this_week_bis_realisatie = int(
                bis_realisatie_dates[sunday_last_week:today].sum()
            )
            this_week_bis_realisatie_yesterday = int(
                bis_realisatie_dates[sunday_last_week:yesterday].sum()
            )
            last_week_bis_realisatie = int(
                bis_realisatie_dates[sunday_two_weeks:sunday_last_week].sum()
            )

            # bis targets
            this_week_bis_target = int(
                bis_target_dates[sunday_last_week:next_sunday].sum()
            )
            last_week_bis_target = int(
                bis_target_dates[sunday_two_weeks:sunday_last_week].sum()
            )

            # graphics
            project_indicators[
                "weekrealisatie"
            ] = thisweek_realisatie_hpend_bullet_chart(project_df, week_target)

            project_indicators[
                "lastweek_realisatie"
            ] = lastweek_realisatie_hpend_bullet_chart(project_df, lastweek_target)

            project_indicators["week_bis_realisatie"] = week_realisatie_bullet_chart(
                this_week_bis_realisatie,
            ...
            project_indicators[
                ...
                last_week_bis_realisatie, None, last_week_bis_target, week_delta=1
            )
            ...
            project_indicators[
                ...
                project=project,
                ratio_HC_HPend_per_project=self.intermediate_results.ratio_HC_HPend_per_project,
            )
            ...
            project_indicators[
                ...
                project=project,
                number_errors_per_project=self.intermediate_results.n_errors_FCBC,
            )
