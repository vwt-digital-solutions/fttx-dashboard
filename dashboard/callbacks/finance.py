...
for client in config.client_config.keys():  # noqa: C901
    ...
    def financial_storage(dropdown_selection, client=client):
        if dropdown_selection:
            ...
            finances = collection.get_document(
                collection="Finance", project=dropdown_selection, client=client
            )
    ...
    def budget_bar_sub_category(click, project, client=client):
        ...
        if "budget-bar-category" in [
            ...
            for point in click.get("points", []):
                data = collection.get_document(
                    collection="Finance", project=project, client=client
                )
                if data:
                    parent = dict(level="categorie", value=point.get("label"))
                    fig = calculate_figure(
                        client, project, data, "sub_categorie", parent
                    )
    ...
    def calculate_figure(client, project, data, level, parent: dict = None):
        actuals_df, budget_df, expected_actuals_df = calculate_level_costs(
            data, level, parent=parent
        )
        if not expected_actuals_df.empty:
            assumed_expenses_df = calculate_assumed_expenses(
                client, project, expected_actuals_df, level, parent
            )
        else:
            assumed_expenses_df = calculate_assumed_expenses(
                client, project, budget_df, level, parent
            )
    ...
    def calculate_level_costs(data, level, parent: dict = None):
        ...
        budget_df = pd.DataFrame(
            data.get("budget", {level: [], parent.get("level"): [], "bedrag": []})
        )
        expected_actuals_df = pd.DataFrame(
            data.get(
                "expected_actuals", {level: [], parent.get("level"): [], "bedrag": []}
            )
        )
        actuals_df = pd.DataFrame(
            data.get(
                "actuals_aggregated", {level: [], parent.get("level"): [], "bedrag": []}
            )
        )

        if parent:
            budget_df = budget_df[budget_df[parent.get("level")] == parent.get("value")]
            expected_actuals_df = expected_actuals_df[
                expected_actuals_df[parent.get("level")] == parent.get("value")
            ]
            actuals_df = actuals_df[
                actuals_df[parent.get("level")] == parent.get("value")
            ]

        budget_df = budget_df[[level, "bedrag"]].groupby(level).sum().reset_index()

        expected_actuals_df = (
            expected_actuals_df[[level, "bedrag"]].groupby(level).sum().reset_index()
        )

        actuals_df = actuals_df[[level, "bedrag"]].groupby(level).sum().reset_index()
        return actuals_df, budget_df, expected_actuals_df

    def calculate_assumed_expenses(
        client, project, expected_actuals_df, level, parent: dict = None
    ):
        progress = collection.get_document(
            "Data", client=client, project=project, data_set="progress"
        )
        n_houses = float(progress.get("totaal"))
        if parent:
            parent_value = (
                parent.get("value") if parent.get("value") != "has" else "hpend"
            )
        else:
            parent_value = None
        progress_percent = {
            ...
            if k
            in (
                [parent_value] if parent else ["schouwen", "montage", "civiel", "hpend"]
            )
        }
        if parent:
            assumed_expenses_df = (
    ...
    def progress_over_time(click, project, client=client):
        ...
        if "budget-bar-category" in [
            ...
            for point in click.get("points", []):
                ...
                finance_data = collection.get_document(
                    collection="Finance", project=project, client=client
                )
                ...
                if finance_data:
                    ...
                    actuals_df = pd.DataFrame(
                        finance_data.get(
                            "actuals", {parent.get("level"): [], "bedrag": []}
                        )
                    )
                    actuals_df = actuals_df[
                        actuals_df[parent.get("level")] == parent.get("value")
                    ...
                    time_series = (
                        actuals_df.groupby("vastlegdatum")["bedrag"]
                        .sum()
                        .sort_index()
                        .cumsum()
                    )
                    ...
                    if "expected_actuals" in finance_data:
                        expected_cost = pd.DataFrame(
                            finance_data.get(
                                "expected_actuals",
                                {parent.get("level"): [], "bedrag": []},
                            )
                        )
                    else:
                        expected_cost = pd.DataFrame(
                            finance_data.get(
                                "budget", {parent.get("level"): [], "bedrag": []}
                            )
                        )

                    expected_cost = expected_cost[
                        expected_cost[parent.get("level")] == parent.get("value")
                    ].bedrag.sum()
