...
import config
from data import api, collection
...
def info_table():
    document = collection.get_document(
        collection="Graphs", client="kpn", graph_name="info_table"
    )
...
def pie_chart(client, key="overview"):
    if key == "overview":
        fig = collection.get_document(
            collection="Data", client=client, graph_name="reden_na_overview"
        )["figure"]
    else:
        fig = collection.get_document(
            collection="Data",
...
def clickbar_lb(drop_selectie, mask_all):
    ...
    fig = dict(
        ...
        layout=dict(
            ...
            title={
                "text": "Status oplevering per fase (LB)<br>[selectie resets na 3x klikken]:",
                "x": 0.5,
            },
...
def update_date():
    ...
    try:
        date_finance_analyse = api.get("/Graphs?id=update_date_kpn_finance")[0]["date"][
            0:-4
        ].replace("T", " ")
    except IndexError:
    ...
    try:
        date_update_baan = api.get("/Graphs?id=update_date_baan_realisation")[0][
            "date"
        ][0:-4].replace("T", " ")
    except IndexError:
    ...
    try:
        date_con = api.get("/Graphs?id=update_date_fiberconnect")[0]["date"][
            0:-4
        ].replace("T", " ")
    except IndexError:
